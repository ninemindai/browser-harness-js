...
```js
async function ddg_instant(query) {
...
let data = await ddg_instant("openai");
// data.Type == 'A'
console.log(data.Heading);          // 'OpenAI'
console.log(data.AbstractText);     // 'OpenAI is an American artificial intelligence research...'
console.log(data.AbstractURL);      // 'https://en.wikipedia.org/wiki/OpenAI'
console.log(data.OfficialWebsite);  // 'https://openai.com/'
console.log(data.Entity);           // 'company'

// Person lookup (skip_disambig resolves D→A automatically)
data = await ddg_instant("elon musk");
console.log(data.Type);                      // 'A' (was 'D' without skip_disambig)
console.log(data.AbstractText.slice(0, 100)); // 'Elon Reeve Musk is a businessman...'
console.log(data.Image);                     // '/i/be2a8644.jpg' — prepend https://duckduckgo.com

// Full image URL
const img_url = data.Image ? `https://duckduckgo.com${data.Image}` : null;
```
...
```js
async function ddg_answer(query) {
  // Returns [answer_text, answer_type]. answer_text is '' if no result.
  const q = encodeURIComponent(query);
  const raw = await http_get(
    `https://api.duckduckgo.com/?q=${q}&format=json&no_html=1&no_redirect=1`
  );
  const data = JSON.parse(raw);
  const ans = data.Answer ?? "";
  // Answer can be an object when it's a widget (calculator, converter) — only string Answers are usable
  return [typeof ans === "string" ? ans : "", data.AnswerType || ""];
}

// Confirmed working instant answers:
let [text, kind] = await ddg_answer("random number");
// text='0.245013228691281 (random number)', kind='rand'

[text, kind] = await ddg_answer("generate password");
// text='ZCsbe8iY (random password)', kind='pw'

[text, kind] = await ddg_answer("ip address");
// text='Your IP address is 73.158.74.222 in San Francisco, California, United States (94121)', kind='ip'

[text, kind] = await ddg_answer("base64 encode hello");
// text='Base64 encode d: aGVsbG8=', kind='base64_conversion'

[text, kind] = await ddg_answer("md5 hash hello");
// text='5d41402abc4b2a76b9719d911017c592', kind='md5'

[text, kind] = await ddg_answer("pi");
// text='3.14159', kind='constants'

[text, kind] = await ddg_answer("today date");
// text='\nS M T W T F S      April 2026\n...|18|...', kind='calendar'

[text, kind] = await ddg_answer("timer 5 minutes");
// text='300', kind='timer'   — returns raw seconds

[text, kind] = await ddg_answer("lorem ipsum");
// text='Ea hic quia corporis. Minus consequuntur...', kind='lorem_ipsum'

// Color lookup — must URL-encode the # sign:
[text, kind] = await ddg_answer("color #FF5733");
// text='Hex: #FF5733 ~ RGBA(255, 87, 51, 1) ~ RGB(100%, 34%, 20%) ~ HSL(11, 100%, 60%) ~ CMYB(0%, 66%, 80%, ...', kind='color_code'
```

**Widget answers return an object, not a string** — `sqrt(144)`, `1 mile in km`, `100 USD in EUR`, and `stopwatch` all return `Answer` as an object like `{from: 'calculator', id: 'calculator', result: '', ...}`. The `result` key is empty — the actual computation happens client-side in a JS widget. Treat object `Answer` values as "not usable via API".
...
Answer           // string or object — instant answer result (see above)
...
Infobox          // object with "content" and "meta" arrays, or "" if no infobox
...
RelatedTopics    // array — see below
Results          // array — official site links (usually 0 or 1 item)
Type             // "A", "D", "C", "N", "E", or ""
...
```js
{
  ...
  "Topics": [                        // array of plain topic objects
...
`Infobox` is `""` (empty string, not `null`, not `{}`) when absent. Always check with `typeof data.Infobox === "object"`.
...
**`Infobox` is `""` not `null` when absent.** Always check with `typeof data.Infobox === "object"` — `if (data.Infobox)` also works since `""` is falsy.
...
**`Answer` can be an object (widget), not a string.** Queries like `1 mile in km`, `100 USD in EUR`, `sqrt(144)`, and `stopwatch` return `Answer` as an object with `{from: 'calculator', result: '', ...}`. The `result` key is empty — the widget computes client-side. Only string `Answer` values are usable via the API.

**`color #RRGGBB` requires URL encoding of `#`.** Using `q=color+#FF5733` returns an HTML page (HTTP redirect). Use `encodeURIComponent("color #FF5733")` which encodes to `color%20%23FF5733`.

**Bang queries without `no_redirect=1` return HTML, not JSON.** `!g python` (without `no_redirect=1`) causes an HTTP 302 to `google.com/search?q=python`. `http_get` follows the redirect and returns Google's HTML — `JSON.parse` fails. Always add `no_redirect=1` when the query might contain bangs.
...
**`OfficialWebsite` is only set for a subset of A-type results.** `machine learning` (Type A) has no `OfficialWebsite`. `openai`, `python programming language`, and `linux kernel` all have one. Always check with `data.OfficialWebsite || ""`.
...
```js
async function ddg_entity(query) {
  // Fetch a DuckDuckGo Instant Answer for a named entity.
  // Returns structured data or null if no result.
  const q = encodeURIComponent(query);
  const raw = await http_get(
    `https://api.duckduckgo.com/?q=${q}&format=json&no_html=1&skip_disambig=1`
  );
  ...
  // Extract infobox as flat object
  if (data.Infobox && typeof data.Infobox === "object") {
    result.infobox = Object.fromEntries(
      data.Infobox.content.map(item => [item.label, item.value])
    );
  }
...
let r = await ddg_entity("openai");
// r.type             === 'A'
// r.heading          === 'OpenAI'
// r.abstract.slice(0, 50) === 'OpenAI is an American artificial intelligence res'
// r.entity           === 'company'
...
r = await ddg_entity("python programming language");
// r.type             === 'A'
