...
```js
const d = JSON.parse(await http_get(`https://thetechgeeks.com/products/${handle}.js`));
...
Product pages are static HTML + one JSON endpoint. `http_get` over `Promise.all` fetches all SKUs in <5s. CDP is wasted here unless you need to click through a cart / checkout flow.
