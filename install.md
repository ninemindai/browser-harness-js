...
description: Install and bootstrap browser-harness (JS) into the current agent, then connect it to the user's real Chrome with minimal prompting.
---

# browser-harness install (JS)

Use this file only for first-time install, reconnect, or cold-start browser bootstrap. For day-to-day browser work, read `SKILL.md`. Always read `helpers.js` after cloning; that is where the functions and expected patterns live.
...
Clone the repo once into a durable location, then install the JS package globally so `browser-harness` works from any directory:
...
That keeps the command global while still pointing at the real repo checkout (npm's `install -g .` symlinks the bin), so when the agent edits `helpers.js` the next `browser-harness` uses the new code immediately. Prefer a stable path like `~/Developer/browser-harness`, not `/tmp`.

Requires Node.js 20.11+.

## Choose which Chrome to attach to (read this first)

The daemon inherits **whatever Chrome it attaches to** — cookies, session tokens, saved logins. Any page the agent visits can, via prompt injection, direct it to read those. See [SECURITY.md](SECURITY.md) for the full trust model.

**Recommended: a dedicated browser install.** Use Chrome Canary, Chrome Beta, or Edge Dev as the "harness browser" — these are separate apps with separate user-data-dirs, so your normal Chrome profile stays untouched. The daemon discovers them automatically (see `PROFILES` in `daemon.js`).

1. Install Chrome Canary (or Edge Dev).
2. Launch it, skip sign-in OR sign into **only the sites the agent legitimately needs**.
3. Follow "Browser bootstrap" below — when it prompts you to tick the remote-debugging checkbox, do it in the dedicated browser, not your main Chrome.

**Using your real Chrome profile is also supported**, but you're explicitly opting into the risk: every cookie in that profile (banking, work SSO, email) is in scope for any page the agent visits. Fine for personal scraping; not fine for anything sensitive.

For parallel agents, give each one its own `BU_NAME` and, if privileges differ, its own browser install.
...
- **Codex**: add this file as a global skill at `$CODEX_HOME/skills/browser-harness/SKILL.md` (often `~/.codex/skills/browser-harness/SKILL.md`). A symlink to this repo's `js/SKILL.md` is fine.
- **Claude Code**: add an import to `~/.claude/CLAUDE.md` that points at this repo's `SKILL.md`, for example `@~/src/browser-harness/js/SKILL.md`.
...
(Run that from the `js/` directory so `$PWD/SKILL.md` resolves to `js/SKILL.md`.)
...
1. Run `npm install` (from `js/`).
   If `browser-harness` is still missing after that, run `command -v browser-harness >/dev/null || npm install -g .`.
...
```
   ...
   Or without a global install, from the `js/` directory: `node run.js <<'JS' ... JS`.
...
```bash
browser-harness <<'JS'
await goto("https://github.com/browser-use/browser-harness");
await wait_for_load();
console.log(await page_info());
JS
```
