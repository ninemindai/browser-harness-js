...
const real_tabs = await list_tabs(false);               // include_chrome=false
...
- `list_tabs()` includes `chrome://newtab/` by default; pass `false` for `include_chrome` when you want only real pages.
